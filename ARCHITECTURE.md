...
> These diagrams mirror the current implementation and are intended to be rendered by GitHub/Markdown viewers that support Mermaid.

### 1) Docker Compose topology (frontend / backend / database / OpenAI)
...
flowchart LR
  ...
  subgraph compose[Docker Compose Environment]
    ...
    subgraph fe[Frontend container]
      direction TB
      fe1[Next.js (App Router)]
      fe2[TypeScript / React]
      fe3[TailwindCSS / Framer Motion]
      fe4[Axios HTTP client\n(JWT interceptor)]
    end

    subgraph be[Backend container]
      direction TB
      be1[FastAPI + Uvicorn]
      be2[API layer\n(auth, presentations, chat, ideas, planner)]
      be3[Service layer\n(RAG, embeddings, intent, file)]
      be4[WebSocket orchestration]
      be5[SQLAlchemy ORM\n(asyncpg)]
      be6[JWT security / Loguru]
    end
    ...
    subgraph db[Database container]
      ...
      db3[Tables\nusers / presentations / slides\nsessions / planner_events]
      db4[asyncpg]
    end
  end
  ...
  fe -- WebSocket --> be
  be -- SQL --> db
  be -- embeddings & chat --> openai
  openai --- openai_models
```

### 2) Upload pipeline (PDF/PPTX → text → embeddings → DB)
...
flowchart TB
  ...
  create[Create Presentation record\nstatus=ANALYZING]

  type{File type?}
  pdf[pypdf\nExtract PDF text\n(per page)]
  pptx[python-pptx\nExtract PPTX text\n+ PDF preview]

  embed[Send slide texts for embeddings\n(batch; max 10 concurrent)]
  openai[(OpenAI API\ntext-embedding-3-small)]
  persist[DB: store slides + vectors]
  doneok{Success?}
  completed[DB: status=COMPLETED]
  failed[DB: status=FAILED\nDelete file from disk\nRollback]
  ready([Presentation ready to use])

  start --> ext --> magic --> size --> sanitize --> ok
  ok -- No --> bad
  ok -- Yes --> save --> create --> type
  type -- PDF --> pdf --> embed
  type -- PPTX --> pptx --> embed
  embed --> openai --> persist --> doneok
  doneok -- Yes --> completed --> ready
  doneok -- No --> failed
```

### 3) RAG chat flow (question → retrieval → answer with page refs)

```mermaid
flowchart LR
  q([User asks a question])
  has{Current slide provided?}
  prio[Include current slide\nas priority context]
  qembed[Convert question to embedding]
  openai_embed[(OpenAI API\ntext-embedding-3-small)]
  vsearch[Vector search on pgvector\ncosine/HNSW ANN]
  pick[Select most relevant slides\n(by similarity score)]
  ctx[Combine slide texts as context]

  sys[Build system prompt\n- respond in user's language\n- require [Sayfa X] refs when using context\n- keep answer concise]
  llm[Send context + question\n to gpt-4o-mini]
  openai_chat[(OpenAI API\ngpt-4o-mini)]
  out[Return response\nanswer + sources + context_used]
  deliver([Deliver answer\n(with [Sayfa X] references)])

  q --> has
  has -- Yes --> prio --> qembed
  has -- No --> qembed
  qembed --> openai_embed --> vsearch --> pick --> ctx --> sys --> llm --> openai_chat --> out --> deliver
```

### 4) Live presentation loop (WS + intent → COMMAND broadcast)

```mermaid
flowchart LR
  start([User starts live presenting])
  ws[WebSocket connection established\nJWT token validated]
  auth{Auth ok?}
  reject[Reject connection\n401 / close 4001]
  sessStart[SESSION_EVENT: START\nDB session created\n(rehearsal/live)]
  sessEnd[SESSION_EVENT: END\nDB session closed\nDuration/metrics saved]
  close([WebSocket closed])

  subgraph loop[Live presentation loop]
    direction TB
    seg[Client sends transcript segment]
    final{is_final?}
    bc1[Broadcast TRANSCRIPT\n(interim UI feedback)]
    intent[Call intent service\n(intent_service)]
    json[LLM returns JSON intent\n(gpt-4o-mini)]
    unknown{Intent UNKNOWN?}
    noop[No action\ncontinue loop]
    cmd[Broadcast COMMAND\n(to all clients)]
    persist[DB: update current_slide_index]
  end

  start --> ws --> auth
  auth -- No --> reject --> close
  auth -- Yes --> sessStart --> seg
  seg --> final
  final -- No --> bc1 --> seg
  final -- Yes --> intent --> json --> unknown
  unknown -- Yes --> noop --> seg
  unknown -- No --> cmd --> persist --> seg
  sessStart -. user stops .-> sessEnd --> close
```
